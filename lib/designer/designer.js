 ...
 */
import { util, containerFields as containers, advanceFields as advancedFields, basicFields, customFields, advanceCustomerFields } from "coder-vform-render"
const { deepClone, generateId, getDefaultFormConfig, overwriteObj } = util
...
export function createDesigner(vueInstance) {
  ...
  return {
    ...
    getFieldWidgetByType(typeName) {
      ...
      //let allWidgets = [...containers, ...basicFields, ...advancedFields, ...customFields]
      foundWidget = findIt(containers) || findIt(basicFields) || findIt(advancedFields) || findIt(customFields)

      if (!foundWidget) {
        advanceCustomerFields.forEach(val => {
          foundWidget = findIt(val);
          if (foundWidget)
            return false;
        })
      }
